...
std::ifstream::pos_type getsize(const char* filename) {
...
//    std::system("( vmstat -n -S m -t 1 >> openfhe_4G_3.txt)&");
//    std::system("(iotop -b -d 0.5 -k >> iotop_openfhe_0125.txt)&");
    //std::system("(dstat -t -gs --mem --vm --ipc >> dstat_memory.txt)&");
    //std::system("(dstat -tdng --file --lock --top-io --top-bio >> dstat_disk.txt)&");
	//std::system("(dstat -ta --top-cpu >> dstat_cpu.txt)&"); 
//	std::system("(dstat -c -C 0,1,2,3,total 4 >>dstat_core_4.txt)&");
  ...
  /*  // export the secret key to know the size
    ...
    std::cout << "secret key size is " << getsize("secret.key") << std::endl;
   */ 
